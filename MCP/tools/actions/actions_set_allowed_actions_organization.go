...
func Actions_set_allowed_actions_organizationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/actions/permissions/selected-actions%s", cfg.BaseURL, org, queryString)
...
func CreateActions_set_allowed_actions_organizationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_orgs_org_actions_permissions_selected-actions",
		mcp.WithDescription("Set allowed actions and reusable workflows for an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_set_allowed_actions_organizationHandler(cfg),
	}
}
