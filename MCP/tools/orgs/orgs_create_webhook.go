...
func Orgs_create_webhookHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/hooks%s", cfg.BaseURL, org, queryString)
...
func CreateOrgs_create_webhookTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_orgs_org_hooks",
		...
		mcp.WithObject("config", mcp.Required(), mcp.Description("Input parameter: Key/value pairs to provide settings for this webhook. [These are defined below](https://docs.github.com/rest/reference/orgs#create-hook-config-params).")),
