...
func Orgs_update_webhookHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/hooks/%s%s", cfg.BaseURL, org, hook_id, queryString)
...
func CreateOrgs_update_webhookTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_orgs_org_hooks_hook_id",
		...
		mcp.WithObject("config", mcp.Description("Input parameter: Key/value pairs to provide settings for this webhook. [These are defined below](https://docs.github.com/rest/reference/orgs#update-hook-config-params).")),
	)
