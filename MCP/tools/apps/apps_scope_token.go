...
func CreateApps_scope_tokenTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_applications_client_id_token_scoped",
		...
		mcp.WithObject("permissions", mcp.Description("Input parameter: The permissions granted to the user-to-server access token.")),
		mcp.WithArray("repositories", mcp.Description("Input parameter: The list of repository names to scope the user-to-server access token to. `repositories` may not be specified if `repository_ids` is specified.")),
		mcp.WithArray("repository_ids", mcp.Description("Input parameter: The list of repository IDs to scope the user-to-server access token to. `repository_ids` may not be specified if `repositories` is specified.")),
		mcp.WithString("target", mcp.Description("Input parameter: The name of the user or organization to scope the user-to-server access token to. **Required** unless `target_id` is specified.")),
		mcp.WithNumber("target_id", mcp.Description("Input parameter: The ID of the user or organization to scope the user-to-server access token to. **Required** unless `target` is specified.")),
