...
func Issues_update_labelHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/labels/%s%s", cfg.BaseURL, owner, repo, name, queryString)
...
func CreateIssues_update_labelTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_repos_owner_repo_labels_name",
		mcp.WithDescription("Update a label"),
		...
		mcp.WithString("color", mcp.Description("Input parameter: The [hexadecimal color code](http://www.color-hex.com/) for the label, without the leading `#`.")),
		mcp.WithString("description", mcp.Description("Input parameter: A short description of the label. Must be 100 characters or fewer.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_update_labelHandler(cfg),
	}
}
