...
func Repos_create_webhookHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/hooks%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_create_webhookTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_hooks",
		mcp.WithDescription("Create a repository webhook"),
		...
		mcp.WithObject("config", mcp.Description("Input parameter: Key/value pairs to provide settings for this webhook. [These are defined below](https://docs.github.com/rest/reference/repos#create-hook-config-params).")),
		...
		mcp.WithString("name", mcp.Description("Input parameter: Use `web` to create a webhook. Default: `web`. This parameter only accepts the value `web`.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_create_webhookHandler(cfg),
	}
}
