...
func Repos_create_releaseHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/releases%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_create_releaseTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_releases",
		...
		mcp.WithBoolean("draft", mcp.Description("Input parameter: `true` to create a draft (unpublished) release, `false` to create a published one.")),
		...
		mcp.WithString("tag_name", mcp.Required(), mcp.Description("Input parameter: The name of the tag.")),
		mcp.WithString("target_commitish", mcp.Description("Input parameter: Specifies the commitish value that determines where the Git tag is created from. Can be any branch or commit SHA. Unused if the Git tag already exists. Default: the repository's default branch (usually `master`).")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_create_releaseHandler(cfg),
	}
}
