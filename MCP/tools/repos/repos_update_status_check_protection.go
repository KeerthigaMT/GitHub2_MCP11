...
func Repos_update_status_check_protectionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/branches/%s/protection/required_status_checks%s", cfg.BaseURL, owner, repo, branch, queryString)
...
func CreateRepos_update_status_check_protectionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_repos_owner_repo_branches_branch_protection_required_status_checks",
		mcp.WithDescription("Update status check protection"),
		...
		mcp.WithArray("contexts", mcp.Description("Input parameter: **Deprecated**: The list of status checks to require in order to merge into this branch. If any of these checks have recently been set by a particular GitHub App, they will be required to come from that app in future for the branch to merge. Use `checks` instead of `contexts` for more fine-grained control.\n")),
		mcp.WithBoolean("strict", mcp.Description("Input parameter: Require branches to be up to date before merging.")),
		mcp.WithArray("checks", mcp.Description("Input parameter: The list of status checks to require in order to merge into this branch.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_update_status_check_protectionHandler(cfg),
	}
}
