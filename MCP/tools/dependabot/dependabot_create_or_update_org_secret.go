...
func Dependabot_create_or_update_org_secretHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/dependabot/secrets/%s%s", cfg.BaseURL, org, secret_name, queryString)
...
func CreateDependabot_create_or_update_org_secretTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_orgs_org_dependabot_secrets_secret_name",
		mcp.WithDescription("Create or update an organization secret"),
		...
		mcp.WithString("key_id", mcp.Description("Input parameter: ID of the key you used to encrypt the secret.")),
		...
		mcp.WithString("visibility", mcp.Required(), mcp.Description("Input parameter: Which type of organization repositories have access to the organization secret. `selected` means only the repositories specified by `selected_repository_ids` can access the secret.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Dependabot_create_or_update_org_secretHandler(cfg),
	}
}
