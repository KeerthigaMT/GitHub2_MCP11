...
func Codespaces_create_with_pr_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pulls/%s/codespaces%s", cfg.BaseURL, owner, repo, pull_number, queryString)
...
func CreateCodespaces_create_with_pr_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_pulls_pull_number_codespaces",
		mcp.WithDescription("Create a codespace from a pull request"),
		...
		mcp.WithString("location", mcp.Description("Input parameter: Location for this codespace. Assigned by IP if not provided")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_create_with_pr_for_authenticated_userHandler(cfg),
	}
}
