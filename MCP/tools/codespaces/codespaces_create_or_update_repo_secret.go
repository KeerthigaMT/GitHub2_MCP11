...
func Codespaces_create_or_update_repo_secretHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/codespaces/secrets/%s%s", cfg.BaseURL, owner, repo, secret_name, queryString)
...
func CreateCodespaces_create_or_update_repo_secretTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_repos_owner_repo_codespaces_secrets_secret_name",
		mcp.WithDescription("Create or update a repository secret"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_create_or_update_repo_secretHandler(cfg),
	}
}
