...
func Pulls_mergeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pulls/%s/merge%s", cfg.BaseURL, owner, repo, pull_number, queryString)
...
func CreatePulls_mergeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_repos_owner_repo_pulls_pull_number_merge",
		...
		mcp.WithString("commit_message", mcp.Description("Input parameter: Extra detail to append to automatic commit message.")),
		mcp.WithString("commit_title", mcp.Description("Input parameter: Title for the automatic commit message.")),
		mcp.WithString("merge_method", mcp.Description("Input parameter: The merge method to use.")),
		mcp.WithString("sha", mcp.Description("Input parameter: SHA that pull request head must match to allow merge.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Pulls_mergeHandler(cfg),
	}
}
