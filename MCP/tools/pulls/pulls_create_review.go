...
func Pulls_create_reviewHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pulls/%s/reviews%s", cfg.BaseURL, owner, repo, pull_number, queryString)
...
func CreatePulls_create_reviewTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_pulls_pull_number_reviews",
		...
		mcp.WithString("commit_id", mcp.Description("Input parameter: The SHA of the commit that needs a review. Not using the latest commit SHA may render your review comment outdated if a subsequent commit modifies the line you specify as the `position`. Defaults to the most recent commit in the pull request when you do not specify a value.")),
		mcp.WithString("event", mcp.Description("Input parameter: The review action you want to perform. The review actions include: `APPROVE`, `REQUEST_CHANGES`, or `COMMENT`. By leaving this blank, you set the review action state to `PENDING`, which means you will need to [submit the pull request review](https://docs.github.com/rest/pulls#submit-a-review-for-a-pull-request) when you are ready.")),
		mcp.WithString("body", mcp.Description("Input parameter: **Required** when using `REQUEST_CHANGES` or `COMMENT` for the `event` parameter. The body text of the pull request review.")),
		mcp.WithArray("comments", mcp.Description("Input parameter: Use the following table to specify the location, destination, and contents of the draft review comment.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Pulls_create_reviewHandler(cfg),
	}
}
